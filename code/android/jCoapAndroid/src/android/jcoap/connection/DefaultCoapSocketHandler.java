...
import android.jcoap.tools.TimeoutHashMap;
...
public class DefaultCoapSocketHandler implements CoapSocketHandler {
	...
	private static Logger logger = Logger
			.getLogger(DefaultCoapSocketHandler.class.getName());
	...
	public DefaultCoapSocketHandler(CoapChannelManager channelManager, int port)
			throws IOException {
	...
	public DefaultCoapSocketHandler(CoapChannelManager channelManager)
			throws IOException {
	...
	protected class WorkerThread extends Thread {
		...
		TimeoutHashMap<MessageKey, Boolean> duplicateMap = new TimeoutHashMap<MessageKey, Boolean>(
		...
		TimeoutHashMap<MessageKey, CoapMessage> retransMsgMap = new TimeoutHashMap<MessageKey, CoapMessage>(
				CoapMessage.ACK_RST_RETRANS_TIMEOUT_MS);
		TimeoutHashMap<MessageKey, CoapMessage> nonConfirmedMsgMap = new TimeoutHashMap<MessageKey, CoapMessage>(
				CoapMessage.ACK_RST_RETRANS_TIMEOUT_MS);
		...
		private void handleIncommingMessage(ByteBuffer buffer,
			...
			CoapMessage msg = new DefaultCoapMessage(buffer.array(), buffer
					.position());
			...
			MessageKey msgKey = new MessageKey(msgID, addr.getAddress(), addr
					.getPort());
			if (duplicateMap.get(msgKey) != null) {
				/*
				 * detected retransmission - sendMessage checks if the message
				 * is null - for CON: send again corresponding ACK or RST, they
				 * are saved in the retransMsgMap when they are send
				 */
				/*
				 * TODO: send message immediately, there is no need to buffer
				 * the message
				 */
			...
			if (channel == null) {
				if ((packetType == CoapPacketType.CON)
					...
					channel = channelManager.createServerChannel(
							DefaultCoapSocketHandler.this, msg, addr
									.getAddress(), addr.getPort());
					if (channel == null) {
						logger.log(Level.INFO, "Reset connection...");
						if (packetType == CoapPacketType.CON) {
							/*
							 * server doesn't accept the connection --> TODO
							 * Reset Connection (Send RST)
							 */
		...
		private void sendMsg(CoapMessage msg) {
			...
			MessageKey msgKey = new MessageKey(msg.getMessageID(), inetAddr,
					port);
			...
			if (packetType == CoapPacketType.CON) {
				...
				TimeoutObject<MessageKey> tObj = new TimeoutObject<MessageKey>(
						msgKey, msg.getTimeout() + System.currentTimeMillis());
			...
			try {
				...
				logger.log(Level.INFO, "Send Msg with ID: "
						+ msg.getMessageID());
			} catch (IOException e) {
				e.printStackTrace();
			}
	...
	private void addChannel(CoapChannel channel) {
		channels.put(new ChannelKey(channel.getRemoteAddress(), channel
	...
	public void removeChannel(CoapChannel channel) {
		channels.remove(new ChannelKey(channel.getRemoteAddress(), channel
				.getRemotePort()));
	}
	...
	/**
	 * @param message
	 *            The message to be sent. This method will give the message a
	 *            new message id!
	 */
	@Override
	...
	public CoapClientChannel connect(CoapClient client,
		...
		CoapClientChannel channel = new DefaultCoapClientChannel(this, client,
