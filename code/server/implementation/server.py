...
class DerpService (coapy.link.LinkValue):
    __derpyString = "ASDASDASDASDASD"

    def process (self, rx_record):
        stringy = self.__derpyString
        msg = coapy.connection.Message(coapy.connection.Message.ACK, code=coapy.OK, payload='%s' % (stringy,))
        rx_record.ack(msg)
...
services.add_service(DerpService('derp'))
services.add_service(TemperatureService('temperature'))
services.add_service(FetchTemperatureService('fetchtemperature'))
services.add_service(SubTemperatureService('subtemperature'))
